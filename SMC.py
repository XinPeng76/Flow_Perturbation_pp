...
if __name__ == '__main__':
    ...
    parser.add_argument('--if_v', type=str2bool, default=True)
    parser.add_argument('--if_K_eps', type=str2bool, default=True)
    parser.add_argument('--sampN', type=int, default=1000)
    parser.add_argument('--eps_type', type=str, default='Gaussian', help='Rademacher or Gaussian')
    parser.add_argument(
    '--method',
    type=int,
    default=-3,
    help='-3: FP at start and end, -2: FP at each step, -1: Jacobian, 0: SNF, 1-n: Hutchinson'
)
    parser.add_argument('--M', type=int, default=1, help='Multiple Noises: Noise Vectors Generated by the FP Method')
    parser.add_argument('--impl', type=str, default='finite', help='finite, jvp, vjp')
    ...
    if args.method == -3:
        if args.M == 1:
            get_log_omega = lambda xT, eps: get_log_omega_FP(xT, eps, exact_dynamics, time_backward, get_energy, tmax=config.Sampler.tmax, fp_impl=args.impl)
        else:
        ...
        args.if_K_eps = True
        args.eps_type = 'Gaussian'
        logger.info(f'method==,{args.method}, using method FP{args.M} at start and end, eps:{args.eps_type}')
        # log_dir加上FP
    elif args.method == -2:
        get_log_omega = lambda xT, eps: get_log_omega_J(xT, eps, exact_dynamics_dSt, time_backward, get_energy, tmax=config.Sampler.tmax, nnoise=args.method,if_fp=True,if_K_eps=args.if_K_eps, impl=args.impl)
        args.M == 1
        args.eps_type = 'Gaussian'
        logger.info(f'method==, {args.method}, using method FP at each step, eps:{args.eps_type}')
    elif args.method == -1:
        ...
        args.M == 1
        args.eps_type = 'Gaussian'
        logger.info(f'method==, {args.method}, using method J, eps:{args.eps_type}')
    elif args.method == 0:
        ...
        args.M == 1
        args.eps_type = 'Gaussian'
        logger.info(f'method==, {args.method}, using method SNF, eps:{args.eps_type}')
    elif args.method > 0:
        #print('args.if_K_eps:',args.if_K_eps)
        get_log_omega = lambda xT, eps: get_log_omega_J(xT, eps, exact_dynamics_dSt, time_backward, get_energy, tmax=config.Sampler.tmax, nnoise=args.method, eps_type=args.eps_type,if_fp=False,if_K_eps=args.if_K_eps, impl=args.impl)
        args.M == 1
        logger.info(f'method==, {args.method}, using method Hutchinson, eps:{args.eps_type}')
    else:
        raise ValueError('method should be -3, -2, -1, 0, or 1-n,-3: FP at start and end, -2: FP at each step, -1: Jacobian, 0: SNF, 1-n: Hutchinson')
    ...
    if args.method in [-3, 0]:
        shape = (config.SMC.n_replicas, config.model.ndim) if args.M == 1 else (args.M, config.SMC.n_replicas, config.model.ndim)
    elif args.method in [1, -2]:
        shape = (config.Sampler.ode_steps, config.SMC.n_replicas, config.model.ndim)
    else:
        shape = (config.Sampler.ode_steps, args.method, config.SMC.n_replicas, config.model.ndim)

    if args.eps_type == 'Gaussian':
        eps_init = torch.randn(shape, device=args.device)
    elif args.eps_type == 'Rademacher':
        eps_init = (torch.randint(0, 2, shape, device=args.device, dtype=torch.float32) * 2 - 1)
    else:
        raise ValueError(f"Unsupported eps_type: {args.eps_type}")
    ...
    for i in range(1, config.SMC.n_steps-1):
        ...
        # mc_step
        xT, eps, log_omega, x0, ux, accept_rate[i] = mc_step(xT, eps, log_omega, x0, ux, Ki_x, Ki_eps, get_log_omega, betai, tmax=config.Sampler.tmax, nmc=config.SMC.nmc, if_K_eps=args.if_K_eps, \
                                                              if_com = if_com, n_particles = n_particles, n_dimensions = n_dimensions, device=args.device,eps_type=args.eps_type)
        ...
        if i %1 == 0:
            logger.info(f'Step {i}/{config.SMC.n_steps}: Ki_x={int(K_x[i].item())}, Ki_eps={int(K_eps[i].item())}')
            logger.info(f'ux_mean: {ux.mean().item()}, log_omega_std: {log_omega.cpu().numpy().std()}, mc_acc_rate: {accept_rate[i].item()}, RC_ratio: {RC_ratio(x0)}, unique_elements: {unique_elements.shape[0]}')
    ...
    logger.info(f'x0 rate:, {RC_ratio(x0)}')
